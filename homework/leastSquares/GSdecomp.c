...
void GS_inverse(gsl_matrix* ortMatrix, gsl_matrix* triangularMatrix, gsl_matrix* inverseMatrix){
  int dimension = triangularMatrix->size1;
  gsl_vector* unitVector = gsl_vector_alloc(dimension);
  gsl_matrix* triangInverse = gsl_matrix_alloc(dimension, dimension);
  ...
  for (int unitVectorId = 0; unitVectorId < dimension; unitVectorId++) {
    ...
    gsl_matrix_set_col(triangInverse, unitVectorId, unitVector);
  }
  gsl_blas_dgemm(CblasNoTrans, CblasTrans, 1.0, triangInverse, ortMatrix, 0, inverseMatrix);
...
void GS_inverse_triang(gsl_matrix* triangularMatrix, gsl_matrix* inverseMatrix){
  ...
  for (int unitVectorId = 0; unitVectorId < dimension; unitVectorId++) {
    ...
    gsl_matrix_set_col(inverseMatrix, unitVectorId, unitVector);
  }
